...
export default function PostItem({data, onClick}: Props) {
    return (
        ...
        >
            ...
            <div className='flex flex-row flex-wrap justify-end gap-1 w-full'>{data.tag.map((tagName, idx) => <p className='rounded-xl bg-neutral-200 text-sm font-semibold py-0.5 px-1' key={idx}>{tagName}</p>)}</div>
        </button>
    )
}